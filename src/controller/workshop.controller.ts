...
export async function authorizeWorkshop(req: Request, res: Response, next: any) {
...
export async function activateWorkshop(req: Request, res: Response) {
    ...
    try {
        const result = await queryCommands.approveWorkshop(new ObjectId(workshopId), true);
        if (result.modifiedCount == 1) {

            const workshop = await queryCommands.getWorkshop(new ObjectId(workshopId));
            ...
            return res.status(400).send({error: "workshop_not_modified", message: "This workshop could not be modified, likely because it does not exist!"});
        }
    } catch (err) {
        Logger.error(err);
        return res.status(400).send({error: "invalid_parameters", message: "The workshop ID is not valid!"})
    }
...
export async function deactivateWorkshop(req: Request, res: Response) {
    ...
    try {
        const result = await queryCommands.approveWorkshop(new ObjectId(workshopId), false);
        if (result.modifiedCount == 1) {
            return res.send({success: true})
        } else {
...
export default {getWorkshop, activateWorkshop, authorizeWorkshop, deactivateWorkshop, getWorkshops}